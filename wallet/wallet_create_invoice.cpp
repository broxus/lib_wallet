...
void CreateInvoiceBox(not_null<Ui::GenericBox *> box, const QString &address, bool testnet, const Ton::Symbol &symbol,
  ...
  const auto amount =  //
      box->addRow(     //
          object_ptr<Ui::InputField>::fromRaw(
              CreateAmountInput(box, rpl::combine(ph::lng_wallet_invoice_number()) | replaceTickerTag(),
                                /*amount*/ 0, symbol)),
          st::walletSendAmountPadding);

  const auto comment =
      box->addRow(object_ptr<Ui::InputField>::fromRaw(CreateCommentInput(box, ph::lng_wallet_invoice_comment())));

  box->addRow(object_ptr<Ui::FlatLabel>(box, ph::lng_wallet_invoice_comment_about(), st::walletSendAbout),
  ...
  const auto collectLink = [=]() -> std::optional<QString> {
    ...
    return TransferLink(address, symbol, *parsed, text);
  };
  const auto submit = [=] {
  ...
  auto amountValue = rpl::single(rpl::empty_value())                                          //
                     | rpl::then(base::qt_signal_producer(amount, &Ui::InputField::changed))  //
                     | rpl::map([=] { return ParseAmountString(amount->getLastText(), tokenDecimals).value_or(0); });

  auto commentValue = rpl::single(rpl::empty_value())                                           //
                      | rpl::then(base::qt_signal_producer(comment, &Ui::InputField::changed))  //
                      | rpl::map([=] { return comment->getLastText(); });

  const auto url =
      box->addRow(object_ptr<Ui::FlatLabel>(box, QString(), st::walletInvoiceLinkLabel), st::walletInvoiceLinkPadding);
  ...
  auto token_name = rpl::combine(ph::lng_wallet_invoice_url_about()) | replaceTickerTag();

  box->addRow(                    //
      object_ptr<Ui::FlatLabel>(  //
          box, (testnet ? ph::lng_wallet_invoice_url_about_test() : token_name), st::walletSendAbout),
  ...
  const auto button = box->addButton(ph::lng_wallet_invoice_share(), submit, st::walletBottomButton);
  button->setTextTransform(Ui::RoundButton::TextTransform::NoTransform);

  const auto parent = button->parentWidget();
  const auto generateLink =
      Ui::CreateChild<Ui::LinkButton>(parent, ph::lng_wallet_invoice_generate_qr(ph::now), st::boxLinkButton);

  rpl::combine(button->geometryValue(), generateLink->widthValue()) |
      rpl::start_with_next(
          [=](QRect button, int width) {
            generateLink->move((parent->width() - width) / 2, button.top() - st::walletGenerateQrLinkTop);
          },
