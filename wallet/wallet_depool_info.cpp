...
rpl::producer<QString> ReplaceAmount(rpl::producer<QString> &&text, rpl::producer<int64> &&amount) {
  return rpl::combine(std::forward<rpl::producer<QString>>(text), std::forward<rpl::producer<int64>>(amount)) |
         rpl::map([](QString &&item, int64 value) {
           return item.replace("{amount}", FormatAmount(value, Ton::Symbol::ton()).full);
         });
}
...
StakeLayout PrepareOrdinaryStakeLayout(int64 id, int64 amount) {
  ...
  const auto formattedAmount = FormatAmount(amount, Ton::Symbol::ton(), FormatFlag::Signed | FormatFlag::Rounded);
...
void RefreshTimeTexts(InvestParamsLayout &layout) {
  layout.lastWithdrawalTimeRaw = base::unixtime::parse(layout.lastWithdrawal);
  layout.lastWithdrawalTime.setText(st::defaultTextStyle,
...
InvestParamsLayout PrepareInvestParamsLayout(bool first, int64 id, const Ton::InvestParams &investParams) {
  ...
  const auto remainingAmount =
      FormatAmount(investParams.remainingAmount, Ton::Symbol::ton(), FormatFlag::Signed | FormatFlag::Rounded);
  ...
  result.owner = Ui::Text::String(AddressStyle(), investParams.owner, _defaultOptions, st::walletAddressWidthMin);
  result.ownerWidth = (AddressStyle().font->spacew / 2) + std::max(addressPartWidth(0, investParams.owner.size() / 2),
                                                                   addressPartWidth(investParams.owner.size() / 2));
  result.ownerHeight = AddressStyle().font->height * 2;

  result.withdrawalPeriod.setText(st::defaultTextStyle, QString{"%1 sec"}.arg(investParams.withdrawalPeriod));

  const auto withdrawalValue =
      FormatAmount(investParams.withdrawalValue, Ton::Symbol::ton(), FormatFlag::Signed | FormatFlag::Rounded);
...
class StakeRow final {
 public:
  explicit StakeRow(int64 id, int64 amount) : _layout{PrepareOrdinaryStakeLayout(id, amount)} {
  }
  ...
  void paint(Painter &p, int x, int y) const {
    ...
    Ui::PaintInlineTokenIcon(Ton::Symbol::ton(), p, diamondLeft, diamondTop, st::normalFont);
  }
...
class InvestParamsRow final {
 public:
  ...
  void paint(Painter &p, int x, int y) const {
    ...
    const auto diamondTop = y + st::walletRowGramsStyle.font->ascent - st::normalFont->ascent;
    const auto diamondLeft = nanoLeft + _layout.remainingAmountNano.maxWidth() + st::normalFont->spacew;
    Ui::PaintInlineTokenIcon(Ton::Symbol::ton(), p, diamondLeft, diamondTop, st::normalFont);
  }
...
DePoolInfo::DePoolInfo(not_null<Ui::RpWidget *> parent, rpl::producer<DePoolInfoState> state) : _widget(parent) {
...
not_null<Ui::VerticalLayout *> EmptySection(not_null<Ui::RpWidget *> parent, Ui::VerticalLayout *layout) {
  const auto wrapper = layout->add(object_ptr<Ui::VerticalLayout>(parent.get()));

  wrapper->add(object_ptr<Ui::FlatLabel>(parent.get(), ph::lng_wallet_depool_info_empty(), st::dePoolInfoEmpty),
...
void DePoolInfo::setupControls(rpl::producer<DePoolInfoState> &&state) {
  ...
  const auto stakesTitle =
      layout->add(object_ptr<Ui::FlatLabel>(&_widget, ph::lng_wallet_depool_info_stakes_title(), st::dePoolInfoTitle),
                  st::dePoolInfoTitlePadding);

  const auto stakesWrapper = layout->add(object_ptr<Ui::RpWidget>(&_widget));
  stakesWrapper->show();

  const auto stakesEmptyLabel = EmptySection(&_widget, layout);

  const auto vestingsTitle =
      layout->add(object_ptr<Ui::FlatLabel>(&_widget, ph::lng_wallet_depool_info_vestings_title(), st::dePoolInfoTitle),
                  st::dePoolInfoTitlePadding);

  EmptySection(&_widget, layout);

  const auto locksTitle =
      layout->add(object_ptr<Ui::FlatLabel>(&_widget, ph::lng_wallet_depool_info_locks_title(), st::dePoolInfoTitle),
                  st::dePoolInfoTitlePadding);
  ...
  _widget.sizeValue()  //
      | rpl::start_with_next(
            [=](QSize size) {
              ...
              stakesEmptyLabel->setMaximumHeight(stakesWrapper->geometry().height() == 0 ? QWIDGETSIZE_MAX : 0);
              layout->adjustSize();
  ...
  stakesWrapper->paintRequest()  //
      | rpl::start_with_next(
            [=](QRect clip) {
              ...
              const auto from = ranges::upper_bound(_stakeRows, clip.top(), ranges::less(), &StakeRow::bottom);
              const auto till =
                  ranges::lower_bound(_stakeRows, clip.top() + clip.height() + 1, ranges::less(), &StakeRow::bottom);
...
rpl::producer<DePoolInfoState> MakeDePoolInfoState(rpl::producer<Ton::WalletViewerState> state,
                                                   rpl::producer<QString> selectedDePool) {
  return rpl::combine(std::move(state), std::move(selectedDePool)) |
         rpl::map([](const Ton::WalletViewerState &state, const QString &address) {
           ...
           if (it != state.wallet.dePoolParticipantStates.end()) {
             return DePoolInfoState{.address = address, .participantState = it->second};
           } else {
