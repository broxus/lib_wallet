...
void ChangePasscodeBox(not_null<Ui::GenericBox *> box,
                       const Fn<void(QByteArray &&old, QByteArray &&now, const Fn<void(QString)> &error)> &submit) {
  ...
  const auto old =
      Ui::CreateChild<Ui::PasswordInput>(inner, st::walletPasscodeInput, ph::lng_wallet_change_passcode_old());
  ...
  inner->widthValue() |
      rpl::start_with_next(
          [=](int width) {
            ...
            old->move((width - old->width()) / 2, st::walletChangePasscodeOldTop);
  ...
  Ui::Connect(old, &Ui::PasswordInput::changed, [=] { error->hide(anim::type::normal); });
