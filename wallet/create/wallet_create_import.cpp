...
Import::Import(Fn<std::vector<QString>(QString)> wordsByPrefix) : Step(Type::Scroll) {
  setTitle(ph::lng_wallet_import_title(Ui::Text::RichLangValue), st::walletImportTitleTop);
  setDescription(ph::lng_wallet_import_description(Ui::Text::RichLangValue));
...
void Import::initControls(Fn<std::vector<QString>(QString)> wordsByPrefix) {
  ...
  const auto noWords =
      Ui::CreateChild<Ui::LinkButton>(inner().get(), ph::lng_wallet_import_dont_have(ph::now), st::defaultLinkButton);
  noWords->setClickedCallback([=] { _actionRequests.fire(Action::NoWords); });

  inner()->sizeValue() |
      rpl::start_with_next(
          [=](QSize size) {
            noWords->move((size.width() - noWords->width()) / 2, contentTop() + st::walletImportNoWordsTop);
