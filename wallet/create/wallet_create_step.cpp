...
QImage AddImageMargins(const QImage &source, QMargins margins) {
  ...
  auto large = QImage(size * pixelRatio, QImage::Format_ARGB32_Premultiplied);
  large.setDevicePixelRatio(pixelRatio);
  large.fill(Qt::transparent);
  {
    auto p = QPainter(&large);
    p.drawImage(QRect({margins.left(), margins.top()}, was), source);
  }
  return large;
}
...
Step::Step(Type type)
    : _type(type), _widget(std::make_unique<Ui::RpWidget>()), _scroll(resolveScrollArea()), _inner(resolveInner()) {
  initGeometry();

  _widget->paintRequest() | rpl::start_with_next([=](QRect clip) { paintContent(clip); }, _widget->lifetime());
}

Step::~Step() = default;

Ui::ScrollArea *Step::resolveScrollArea() {
  return (_type == Type::Scroll) ? Ui::CreateChild<Ui::ScrollArea>(_widget.get(), st::walletScrollArea) : nullptr;
}

not_null<Ui::RpWidget *> Step::resolveInner() {
  return _scroll ? _scroll->setOwnedWidget(object_ptr<Ui::RpWidget>(_scroll)).data()
...
void Step::initGeometry() {
  _widget->sizeValue() | rpl::start_with_next(
                             [=](QSize size) {
                               ...
                               _inner->setGeometry(0, 0, size.width(), std::max(desiredHeight(), size.height()));
                             },
                             _inner->lifetime());
}
...
void Step::showFinished() {
  if (_slideAnimation.lottieNow) {
    ...
    _lottie->setGeometry(lottieGeometry(_lottiePosition, contentTop(), _lottieSize));
  }
...
rpl::producer<Qt::KeyboardModifiers> Step::nextClicks() const {
  return !_nextButton ? rpl::producer<Qt::KeyboardModifiers>()
                      : _nextButton->clicks() | rpl::map([=] { return _nextButton->clickModifiers(); });
}
...
Step::SlideAnimationData Step::prepareSlideAnimationData() {
  ...
  if (_lottie) {
    ...
    result.lottiePosition = _lottiePosition + QPoint(0, contentTop() - scrollTop);
...
QImage Step::prepareSlideAnimationContent() const {
  ...
  const auto contentWidth = std::max(_description->naturalWidth(), st::walletWindowSize.width());
  const auto contentRect =
      QRect((inner()->width() - contentWidth) / 2, contentTop, contentWidth, animationContentBottom() - contentTop);
...
int Step::slideAnimationContentTop() const {
  ...
  return std::max(_title->y(), _scroll ? _scroll->scrollTop() : 0);
}

int Step::animationContentBottom() const {
  const auto bottom = contentTop() + desiredHeight();
  return _scroll ? std::min(bottom, _scroll->scrollTop() + _scroll->height()) : bottom;
}
...
void Step::setTitle(rpl::producer<TextWithEntities> text, int top) {
  _title.emplace(inner(), std::move(text), st::walletStepTitle);

  if (!top) {
    top = (_type == Type::Scroll) ? st::walletStepScrollTitleTop : st::walletStepTitleTop;
  }
...
void Step::setDescription(rpl::producer<TextWithEntities> text, int top) {
  ...
  _description.emplace(inner(), std::move(text), st::walletStepDescription);

  if (!top) {
    top = (_type == Type::Scroll) ? st::walletStepScrollDescriptionTop : st::walletStepDescriptionTop;
  }
  _title->geometryValue() | rpl::start_with_next(
                                [=](QRect geometry) {
...
void Step::showNextButton(rpl::producer<QString> text) {
  ...
  _nextButton->setTextTransform(Ui::RoundButton::TextTransform::NoTransform);
  inner()->sizeValue() |
      rpl::start_with_next(
          [=](QSize size) {
            const auto skip = (_type == Type::Scroll) ? st::walletWordsNextBottomSkip
                                                      : (st::walletStepHeight - st::walletStepNextTop);
            _nextButton->move((size.width() - _nextButton->width()) / 2, contentTop() + desiredHeight() - skip);
          },
...
void Step::showBelowNextButton(object_ptr<Ui::RpWidget> widget) {
  ...
  inner()->sizeValue() | rpl::start_with_next(
                             [=](QSize size) {
                               _belowNextButton->move((size.width() - _belowNextButton->width()) / 2,
                                                      _nextButton->y() + _nextButton->height());
                             },
                             _belowNextButton->lifetime());
}

void Step::adjustSlideSnapshots(SlideAnimationData &was, SlideAnimationData &now) {
  const auto pixelRatio = style::DevicePixelRatio();
  ...
  was.contentTop = now.contentTop = std::min(was.contentTop, now.contentTop);
}
...
void Step::showAnimatedSlide(not_null<Step *> previous, Direction direction) {
  ...
  _slideAnimation.slide->setSnapshots(Ui::PixmapFromImage(std::move(was.content)),
  ...
  _slideAnimation.slide->start((direction == Direction::Backward), [=] { slideAnimationCallback(); },
                               st::slideDuration);
...
void Step::slideAnimationCallback() {
  ...
  if (_slideAnimation.lottieWas) {
    const auto shown = 1. - (forward ? state.leftProgress : state.rightProgress);
    ...
    _slideAnimation.lottieWas->setGeometry(
        lottieGeometry(_slideAnimation.lottieWasPosition, (1. - scale) * fullHeight / 2., height)
            .translated(forward ? -delta : delta, 0));
  }
  if (_slideAnimation.lottieNow) {
    const auto shown = forward ? state.rightProgress : state.leftProgress;
    ...
    _slideAnimation.lottieNow->setGeometry(
        lottieGeometry(_slideAnimation.lottieNowPosition, (1. - scale) * fullHeight / 2., height)
            .translated(forward ? delta : -delta, 0));
  }
  _widget->update();
}
...
void Step::showLottie(const QString &name, QPoint position, int size) {
  _lottie = std::make_unique<Ui::LottieAnimation>(inner(), Ui::LottieFromResource(name));
  _lottiePosition = position;
  _lottieSize = size;

  inner()->sizeValue() | rpl::filter([=] { return (_lottie->parent() == inner()); }) |
      rpl::start_with_next(
          [=](QSize size) { _lottie->setGeometry(lottieGeometry(_lottiePosition, contentTop(), _lottieSize)); },
          inner()->lifetime());
}
...
QRect Step::lottieGeometry(QPoint position, int top, int size) const {
  return QRect(position.x() + (inner()->width() - size) / 2, position.y() + top, size, size);
}
