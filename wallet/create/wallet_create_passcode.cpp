...
Passcode::Passcode(rpl::producer<QString> syncing) : Step(Type::Default) {
  ...
  setDescription(ph::lng_wallet_set_passcode_description(Ui::Text::RichLangValue));
...
void Passcode::initControls(rpl::producer<QString> syncing) {
  showLottie("lock", st::walletStepPasscodeLottiePosition, st::walletStepPasscodeLottieSize);
  stopLottieOnLoop();

  const auto enter =
      Ui::CreateChild<Ui::PasswordInput>(inner().get(), st::walletPasscodeInput, ph::lng_wallet_set_passcode_enter());
  const auto repeat =
      Ui::CreateChild<Ui::PasswordInput>(inner().get(), st::walletPasscodeInput, ph::lng_wallet_set_passcode_repeat());

  inner()->sizeValue() |
      rpl::start_with_next(
          [=](QSize size) {
            enter->move((size.width() - enter->width()) / 2, contentTop() + st::walletSetPasscodeEnterTop);
            repeat->move((size.width() - repeat->width()) / 2, contentTop() + st::walletSetPasscodeRepeatTop);
          },
          inner()->lifetime());

  showNextButton(ph::lng_wallet_continue());

  const auto NonEmptyString = [](const QString &text) { return !text.isEmpty(); };
  auto syncingLabel = object_ptr<Ui::PaddingWrap<Ui::FadeWrap<Ui::FlatLabel>>>(
      inner().get(),
      object_ptr<Ui::FadeWrap<Ui::FlatLabel>>(
          inner().get(), object_ptr<Ui::FlatLabel>(inner().get(), rpl::duplicate(syncing) | rpl::filter(NonEmptyString),
  ...
  syncingLabel->wrapped()->toggleOn(std::move(syncing) | rpl::map(NonEmptyString));
