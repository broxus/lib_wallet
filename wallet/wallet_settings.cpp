...
[[nodiscard]] QString FormatVersion(int version) {
  ...
  return patch ? QString("%1.%2.%3").arg(major).arg(minor).arg(patch) : QString("%1.%2").arg(major).arg(minor);
}
...
[[nodiscard]] QString FormatDownloadProgress(int64 ready, int64 total) {
  ...
  return ph::lng_wallet_downloaded(ph::now)
...
void SetupUpdate(not_null<Ui::GenericBox *> box, not_null<UpdateInfo *> info) {
  ...
  const auto texts = Ui::CreateChild<rpl::event_stream<QString>>(box.get());
  const auto downloading = Ui::CreateChild<rpl::event_stream<bool>>(box.get());
  const auto version =
      ph::lng_wallet_settings_version(ph::now).replace("{version}", FormatVersion(info->currentVersion()));
  const auto toggle = box->addRow(
      object_ptr<Ui::SettingsButton>(box, ph::lng_wallet_settings_autoupdate(), st::walletSettingsUpdateToggle),
      QMargins());
  const auto label = Ui::CreateChild<Ui::FlatLabel>(toggle, texts->events(), st::walletSettingsUpdateState);
  ...
  const auto check = inner->add(
      object_ptr<Ui::SettingsButton>(inner, ph::lng_wallet_settings_check(), st::defaultSettingsButton), QMargins());
  const auto update = Ui::CreateChild<Ui::SettingsButton>(check, ph::lng_wallet_settings_update() | Ui::Text::ToUpper(),
  ...
  rpl::combine(toggle->widthValue(), label->widthValue()) |
      rpl::start_with_next(
          [=] {
            label->moveToLeft(st::walletSettingsUpdateStatePosition.x(), st::walletSettingsUpdateStatePosition.y());
          },
          label->lifetime());
  ...
  const auto showDownloadProgress = [=](int64 ready, int64 total) {
    auto text =
        ph::lng_wallet_settings_downloading(ph::now).replace("{progress}", FormatDownloadProgress(ready, total));
  ...
  options->toggleOn(rpl::combine(toggle->toggledValue(), downloading->events_starting_with_copy(downloadingNow)) |
                    rpl::map([](bool check, bool downloading) { return check && !downloading; }));
  ...
  info->isLatest()  //
      | rpl::start_with_next(
            ...
            options->lifetime());
  info->progress()  //
      | rpl::start_with_next([=](UpdateProgress progress) { showDownloadProgress(progress.already, progress.size); },
                             options->lifetime());
  info->failed()  //
      | rpl::start_with_next(
            [=] {
  ...
  check->addClickHandler([=] { info->check(); });
  update->addClickHandler([=] { info->install(); });
}
...
void SettingsBox(not_null<Ui::GenericBox *> box, const Ton::Settings &settings, UpdateInfo *updateInfo,
  ...
  const auto download =
      box->addRow(  //
             object_ptr<Ui::SettingsButton>(box, ph::lng_wallet_settings_update_config(), st::defaultSettingsButton),
             QMargins())
          ->toggleOn(downloadOn->value());

  const auto filenames = box->lifetime().make_state<rpl::event_stream<QString>>();
  auto text = rpl::combine(ph::lng_wallet_settings_config_from_file(), filenames->events_starting_with(QString())) |
              rpl::map([](const QString &phrase, const QString &name) { return name.isEmpty() ? phrase : name; });
  const auto custom =  //
      box->addRow(     //
             object_ptr<Ui::SlideWrap<Ui::SettingsButton>>(
                 box, object_ptr<Ui::SettingsButton>(box, std::move(text), st::defaultSettingsButton)),
             QMargins())
          ->setDuration(0);
  ...
  const auto chooseFromFile = [=] {
    ...
    const auto path = QFileDialog::getOpenFileName(box->window(), QString(), QString(), filter);
  ...
  custom->entity()->setClickedCallback([=] {
    ...
    base::call_delayed(st::defaultSettingsButton.ripple.hideDuration, custom, [=] {
      *choosing = false;
      chooseFromFile();
    });
  });
  ...
  const auto heightDelta = st::defaultSettingsButton.padding.top() + st::defaultSettingsButton.height +
                           st::defaultSettingsButton.padding.bottom() - st::walletInput.heightMin;
  const auto url = box->addRow(
      object_ptr<Ui::SlideWrap<Ui::InputField>>(
          box, object_ptr<Ui::InputField>(box, st::walletInput, ph::lng_wallet_settings_config_url(), was.configUrl),
          QMargins(0, 0, 0, heightDelta)),
      (st::boxRowPadding + QMargins(0, 0, 0, st::walletSettingsBlockchainNameSkip)));
  ...
  const auto validate = [=] {
    ...
    if (updated.isEmpty() || (net->value() == 1 && !updated.compare("mainnet", Qt::CaseInsensitive))) {
      name->showError();
      return false;
    } else if (!custom->toggled() && url->entity()->getLastText().trimmed().isEmpty()) {
