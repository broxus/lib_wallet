...
void DePoolWithdrawBox(
	...
	rpl::duplicate(state) | rpl::start_with_next([=](const Ton::WalletState &state) {
	...
	const auto withdrawalType = box->lifetime().make_state<rpl::variable<WithdrawalType>>(
		static_cast<WithdrawalType>(invoice.all));
	...
	const auto amountWrapper = box->addRow(
	...
	auto balanceText = rpl::combine(
		ph::lng_wallet_withdraw_locked(),
		totalStake->value()
	) | rpl::map([=](QString &&phrase, int64 value) {
		return phrase.replace(
			"{amount}",
			FormatAmount(std::max(value, 0LL), defaultToken, FormatFlag::Rounded).full);
	});
	...
	const auto showError = crl::guard(box, [=](DePoolWithdrawField field) {
		switch (field) {
			case DePoolWithdrawField::Amount: amount->showError(); return;
		}
	...
	const auto submit = [=] {
		...
		if (withdrawalType->current() == WithdrawalType::All) {
			...
			const auto parsed = ParseAmountString(amount->getLastText(), Ton::countDecimals(defaultToken));
			if (!parsed) {
				amount->showError();
				return;
			}
	...
	auto text = rpl::combine(
		...
		rpl::single(withdrawalType->current())
	) | rpl::then(
		rpl::combine(
			rpl::single(
				rpl::empty_value()
		...
		const auto value = ParseAmountString(text, Ton::countDecimals(defaultToken)).value_or(0);
		if (type == WithdrawalType::All) {
			return ph::lng_wallet_withdraw_button_all();
		} else if (value > 0) {
			return rpl::combine(
				ph::lng_wallet_withdraw_button_amount(),
				ph::lng_wallet_grams_count(
					FormatAmount(value, defaultToken).full,
					defaultToken)()
			) | rpl::map([=](QString &&text, const QString &amount) {
	...
	const auto checkFunds = [=](const QString &amount) {
		if (const auto value = ParseAmountString(amount, Ton::countDecimals(defaultToken))) {
	...
	totalStake->value() | rpl::start_with_next([=](int64 value) {
