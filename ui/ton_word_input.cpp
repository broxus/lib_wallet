...
TonWordInput::TonWordInput(not_null<QWidget *> parent, const style::InputField &st, int index,
  ...
  base::install_event_filter(_word.data(), [=](not_null<QEvent *> e) {
    ...
    const auto direction = ((ev->key() == Qt::Key_Tab) && !(ev->modifiers() & Qt::ShiftModifier))
...
void TonWordInput::setupSuggestions() {
  base::qt_signal_producer(_word.data(), &InputField::changed) |
      rpl::start_with_next(
          [=] {
            _chosen = false;
  ...
  focused() | rpl::filter([=] { return !_chosen; }) |
      rpl::start_with_next([=] { showSuggestions(word()); }, _word->lifetime());
...
void TonWordInput::createSuggestionsWidget() {
  _suggestions = std::make_unique<TonWordSuggestions>(_word->parentWidget());

  _suggestions->chosen() | rpl::start_with_next(
                               [=](QString word) {
                                 _chosen = true;
                                 _word->setText(word);
                                 _word->setFocus();
  ...
  _suggestions->hidden() | rpl::start_with_next([=] { _suggestions = nullptr; }, _suggestions->lifetime());

  _word->geometryValue() |
      rpl::start_with_next(
          [=](QRect geometry) {
            _suggestions->setGeometry(geometry.topLeft() + QPoint(0, geometry.height()), geometry.width());
          },
          _suggestions->lifetime());

  _word->events() | rpl::filter([](not_null<QEvent *> e) { return (e->type() == QEvent::KeyPress); }) |
      rpl::map([=](not_null<QEvent *> e) { return static_cast<QKeyEvent *>(e.get())->key(); }) |
      rpl::start_with_next(
          [=](int key) {
            if (key == Qt::Key_Up) {
              _suggestions->selectUp();
  ...
  blurred() | rpl::start_with_next([=] { _suggestions->hide(); }, _suggestions->lifetime());
}
...
rpl::producer<> TonWordInput::submitted() const {
  return base::qt_signal_producer(_word.data(), &InputField::submitted) | rpl::filter([=] {
           if (_suggestions) {
             _suggestions->choose();
             return false;
           }
